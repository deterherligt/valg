...
def test_party_votes_joined_by_id():
    """party_votes joins to opstillingskredse by id."""
    from valg.models import get_connection, init_db
    ...
    conn.execute("INSERT OR REPLACE INTO opstillingskredse (id, name, storkreds_id) VALUES ('403564','Østerbro','1')")
    conn.execute("INSERT OR REPLACE INTO parties (id, letter, name) VALUES ('A','A','Parti A')")

    snap = "2026-03-24T21:00:00"
    conn.commit()
    conn.execute("PRAGMA foreign_keys=OFF")
    conn.execute("INSERT INTO party_votes (opstillingskreds_id, party_id, votes, snapshot_at) VALUES ('403564','A',5000,?)", (snap,))
    conn.commit()

    national, storkreds, kredsmandater = get_seat_data(conn)
    assert national.get("A", 0) == 5000
