...
def _extract_parties(conn, data: dict) -> None:
    """Extract party names from partistemmefordeling or kandidat-data and upsert into parties table."""
    for party in (data.get("IndenforParti") or []):
        ...
        # Only overwrite name if we have a real one (not just the letter)
...
def process_raw_file(
    ...
    # Extract parties from partistemmefordeling or kandidat-data (2026 format has party names inline)
